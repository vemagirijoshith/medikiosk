...
def build_physician_review_packet(db: Session, patient: Patient, encounter_id: int | None) -> dict[str, Any]:
    ...
    return {"patient": {"patient_id": patient.id, "name": patient.name, "age": patient.age, "sex": patient.gender},
        "encounter": {"encounter_id": encounter.id, "started_at": encounter.started_at, "chief_complaint": encounter.chief_complaint} if encounter else None,
        ...
        "red_flags": [], "documents": [{"document_id": x.id, "document_type": x.document_type, "filename": x.file_name, "ocr_status": x.ocr_status} for x in documents],
        "ocr_findings": ocr_findings, "medical_extraction": medical_extraction, "document_intelligence": intelligence,
        ...
        "consultations": [{"consultation_id": x.id, "encounter_id": x.encounter_id, "review_status": x.review_status, "physician_notes": x.physician_notes, "existing_notes": x.notes, "physician_id": x.physician_id, "reviewed_at": x.reviewed_at, "physician_id_authenticated": False} for x in consultations_query.all()],
